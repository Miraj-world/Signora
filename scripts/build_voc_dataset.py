...
def product_rules(filename, text, title=""):
    ...
    if "echo" in filename:
        target = "Amazon Echo / Alexa"
        family = "Amazon Echo / Alexa"
        if "echo show" in blob or re.search(r"\bshow\b", blob):
            model = "Echo Show"
            conf = 0.88
            reason = "Echo Show mention in broader Amazon Echo/Alexa corpus."
        elif "echo dot" in blob or re.search(r"\bdot\b", blob):
            model = "Echo Dot"
            conf = 0.9
            reason = "Echo Dot mention in broader Amazon Echo/Alexa corpus."
        elif any(x in blob for x in ["alexa", "echo", "routine", "drop in", "by the way", "ziggy"]):
            model = "General Alexa"
            conf = 0.82
            reason = "General Alexa/Echo discussion belongs to broader Amazon Echo/Alexa family."
        else:
            model = "Unknown"
            conf = 0.55
            reason = "Provided Echo CSV row without enough text to identify a specific model."
        return target, "Amazon", family, model, None, None, True, conf, reason, "accepted"
    if "sony" in filename:
        family = "Sony WH-1000X series"
        xm4_owned = re.search(r"\b(my|our|owned|had|have|using|use)\s+(?:sony\s+)?(?:wh-)?1000xm4s?\b|\b(my|our|owned|had|have|using|use)\s+xm4s?\b", blob)
        if any(x in blob for x in ["wf-1000xm5", "wf1000xm5"]):
            return "Sony WF-1000XM5", "Sony", "WF earbuds", "WF-1000XM5", "XM5", None, False, 0.15, "Mentions WF earbuds, not WH headphones.", "quarantined_wrong_product"
        if xm4_owned or any(x in blob for x in ["wh-1000xm4", "wh1000xm4", "1000xm4", "xm4"]):
            return "Sony WH-1000XM4", "Sony", family, "WH-1000XM4", "XM4", None, True, 0.86, "XM4/WH-1000XM4 mention normalized separately from XM5.", "accepted"
        if any(x in blob for x in ["wh-1000xm6", "wh1000xm6", "1000xm6", "xm6"]):
            return "Sony WH-1000XM6", "Sony", family, "WH-1000XM6", "XM6", None, True, 0.88, "XM6/WH-1000XM6 mention normalized separately from XM5.", "accepted"
        if any(x in blob for x in ["wh-1000xm5", "wh1000xm5", "1000xm5", "xm5"]):
            return "Sony WH-1000XM5", "Sony", family, "WH-1000XM5", "XM5", None, True, 0.88, "XM5/WH-1000XM5 mention.", "accepted"
        if any(x in blob for x in ["wh-1000x", "wh1000x", "1000x", "sony", "headphone"]):
            return "Sony WH-1000X series", "Sony", family, None, None, None, True, 0.55, "Sony WH/headphone discussion normalized to the WH-1000X series.", "accepted"
        return "Sony WH-1000X series", None, None, None, None, None, False, 0.05, "No target product evidence.", "quarantined_off_topic"
    target = "Wyze Cam v3"
...
def reddit_thread_id(url):
    match = re.search(r"reddit\.com/r/[^/]+/comments/([^/]+)/?", url or "")
    return match.group(1) if match else None


def reddit_root_post_id(row, thread_id):
    source_id = row.get("id")
    if source_id and source_id.startswith("t3_"):
        return source_id
    return f"t3_{thread_id}" if thread_id else None


def reddit_parent_id(row):
...
def reddit_is_root_post(row):
    source_id = row.get("id") or ""
    data_type = (row.get("dataType") or "").lower()
    return data_type == "post" or source_id.startswith("t3_")
...
def audit_public_csvs():
    ...
    for path in sorted(PUBLIC_INPUT.glob("*.csv")):
        ...
        for r, t in zip(rows, texts):
            ...
            thread_id = reddit_thread_id(r.get("url"))
            is_root_post = reddit_is_root_post(r)
            ...
            item = {
                ...
                "source_data_type": r.get("dataType"),
                "thread_id": thread_id,
                "root_post_id": reddit_root_post_id(r, thread_id),
                "parent_id": reddit_parent_id(r),
        ...
        md += [
            ...
            f"- Product identity: {audit['records_matching_target_product']} normalized/accepted, {audit['records_mentioning_another_product']} wrong product, {audit['validation_status_counts']}",
...
def public_reference_files():
    checked = now_iso()
    products = [
        {"product_id": "amazon_echo_alexa", "product_name": "Amazon Echo / Alexa", "brand": "Amazon", "category": "smart speaker / voice assistant", "source_url": "provided_csv_sample", "retrieved_at": checked},
        ...
        {"product_id": "amazon_echo_show", "product_name": "Amazon Echo Show", "brand": "Amazon", "category": "smart display", "source_url": "provided_csv_sample", "retrieved_at": checked},
        ...
        {"product_id": "sony_wh1000xm6", "product_name": "Sony WH-1000XM6", "brand": "Sony", "category": "headphones", "source_url": "provided_csv_sample", "retrieved_at": checked},
        {"product_id": "sony_wh1000xm4", "product_name": "Sony WH-1000XM4", "brand": "Sony", "category": "headphones", "source_url": "provided_csv_sample", "retrieved_at": checked},
        {"product_id": "sony_wh1000x_series", "product_name": "Sony WH-1000X series", "brand": "Sony", "category": "headphones", "source_url": "provided_csv_sample", "retrieved_at": checked},
...
def reports(audits, metrics):
    contamination = []
    for a in audits:
        contamination.append(f"- `{a['file_name']}`: normalized/accepted {a['records_matching_target_product']} / {a['record_count']}; wrong-product {a['records_mentioning_another_product']}; statuses {a['validation_status_counts']}.")
    missing = []
    for a in audits:
    ...
    md = [
        ...
        "- Public Reddit rows are normalized to product family/model where needed: Amazon Echo/Alexa is treated as a broader family, and Sony XM4/XM5/XM6/WH-1000X series mentions are separated rather than forced into XM5.",
        *contamination,
        "",
